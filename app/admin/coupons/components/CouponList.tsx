    ...
    <AdminTable>
      <thead className="bg-(--gray-50) border-b border-(--gray-200)">
        <tr>
          ...
          <th className="px-6 py-3 text-sm font-semibold text-(--gray-900)">Link</th>
        ...
        {coupons.map((coupon) => {
          ...
          return (
            <tr key={coupon.id} className="hover:bg-(--gray-50) transition-colors">
              ...
              <td className="px-6 py-4">
                {coupon.destinationLink ? (
                  <a href={coupon.destinationLink} target="_blank" rel="noopener noreferrer" className="text-(--primary) hover:underline text-xs truncate max-w-[100px] block">
                    {coupon.destinationLink}
                  </a>
                ) : (
