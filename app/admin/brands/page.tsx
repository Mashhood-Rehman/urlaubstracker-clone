            ...
            {showForm && (
                <form onSubmit={handleSubmit} className="bg-(--white) p-6 rounded-lg shadow-md space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className="block text-sm font-medium mb-1 text-(--gray-700)">Website Link</label>
                            <input
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            className="w-full px-4 py-2 border border-(--gray-200) rounded-lg h-24 bg-(--white)"
                        />
                    </div>
                    <div className="flex justify-end gap-2">
                        <button
                            ...
                            className="px-4 py-2 text-(--gray-600) hover:text-(--foreground)"
                        >
                            Cancel
                        </button>
                        <button
                            disabled={loading}
                            type="submit"
                            className="px-4 py-2 bg-(--primary) text-(--white) rounded-lg flex items-center gap-2 hover:opacity-90"
                        >
                    ...
                    {brands.map((brand) => (
                        <div key={brand.id} className="bg-(--white) p-6 rounded-xl shadow-sm border border-(--gray-100) hover:shadow-md transition-shadow">
                            <div className="flex justify-between items-start mb-4">
                                <div className="w-16 h-16 bg-(--muted) rounded-lg flex items-center justify-center overflow-hidden border border-(--gray-200)">
                                ...
                                <div className="flex gap-1">
                                    <button onClick={() => handleEdit(brand)} className="p-2 text-(--primary) hover:bg-(--primary)/10 rounded-lg">
                                        <icons.Edit2 className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => handleDelete(brand.id)} className="p-2 text-(--error) hover:bg-(--error)/10 rounded-lg">
                            ...
                            <div className="flex items-center justify-between pt-4 border-t border-(--gray-100)">
                                ...
                                {brand.websiteLink && (
                                    <a href={brand.websiteLink} target="_blank" rel="noopener noreferrer" className="text-(--primary) hover:underline flex items-center gap-1 text-sm">
